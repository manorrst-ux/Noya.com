...
export function youtubeConfigured() { const env = getEnv(); return Boolean(env.YOUTUBE_CLIENT_ID && env.YOUTUBE_CLIENT_SECRET && env.YOUTUBE_REDIRECT_URI); }
export function youtubeAuthorizeUrl(state: string) {
  const env = getEnv();
  if (!env.YOUTUBE_CLIENT_ID || !env.YOUTUBE_CLIENT_SECRET || !env.YOUTUBE_REDIRECT_URI) throw new Error("YouTube OAuth is not configured");
